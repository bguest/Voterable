...
# module Voterable
   module Voteable
      ...
      included do
         ...
         has_many    :votes,  as: :voteable, dependent: :delete
         embeds_many :tallys, as: :tallyable
      ...
      module ClassMethods
         ...
         def voteback(klass = self, options = nil)
            VOTEBACK[name] ||= {}
            VOTEBACK[name][klass.name] ||= options
         end

         def options(value)
            VOTEABLE[name][name][value]
         end

         def vtback(value)
            VOTEBACK[name][name][value]
         end
